module src.Data.BCBPV where 
    ...
    module _ {ℓ ℓ' : Level}(SMC : StrictMonCategory ℓ ℓ') where
        ...
        module _ {isSetCob : isSet (ob C)} where 
            ...
            open Ran C isSetCob hiding (Inc*)
            ...
            module _ (F : Functor 𝓥 𝓒)
                ...
                module _ 
                    (ob₁ : {X : ob 𝓥}{Y : ob 𝓒} → Iso (𝓥 [ X , U .F-ob Y ]) 𝓞[ X , Y ] )
                    (ob₂ : {X : ob 𝓥}{Y : ob 𝓒} → Iso 𝓞[ X , Y ] (𝓒 [ F .F-ob X , Y ]) ) where 
                    ...
                    sep A B .F-seq f g = funExt λ end → funExt λ w₃  → funExt λ Aw₃ → cong (λ h → B .F-hom h _) 
                        ...
                        --cong (λ h → ⨂c .F-hom h) (≡-× refl {!   !})

                    module cbpvSepUP {P Q : ob 𝓥}{R : ob 𝓒}where 

                    -- ⊸UP' : Iso (𝓥× [ P ⨂Ext Q , R ∘F (⨂c ^opF) ]) (𝓥 [ P , Q ⊸ R ]) 
                        --⊸UP' 
                    -- ⊸UP' : Iso 𝓞[ P ⨂Ext Q , R ] 𝓞[ P , sep Q R ] 
                    -- ⊸UP' = ?

                        
                        hmm : Iso 𝓞[ P ⨂ᴰ Q , R ] (𝓥 [ P ⨂ᴰ Q , U .F-ob R ]) 
                        hmm = {!   !}  
