...
\begin{prooftree}
    \AxiomC{$\Gamma \vdash A \; small$}
    \AxiomC{$\Gamma \vdash A' \; small$}
    \BinaryInfC{$\Gamma \vdash A \times A' \; small$}
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{$\Gamma \vdash A \;small$}
    \UnaryInfC{$\Gamma \vdash \lceil ddd \rceil : Val$}
\end{prooftree}
...
\begin{prooftree}
    ...
    \UnaryInfC{$\Gamma \vdash \lfloor A \rfloor \; small$}
\end{prooftree}

These \textit{quoting} ($\lceil \_ \rceil$) and \textit{unquoting} ($\lfloor \_ \rfloor$) operations form an isomorphism.
(between what? Val and ??)
That is to say, we have the two equations
\begin{prooftree}
    \AxiomC{$\Gamma \vdash A \; small$}
    \UnaryInfC{$\Gamma \vdash A = \lfloor \lceil A \rceil \rfloor$}
\end{prooftree}

\begin{prooftree}
    \AxiomC{$\Gamma \vdash A : Val$}
    \UnaryInfC{$\Gamma \vdash A = \lceil \lfloor A \rfloor \rceil$}
\end{prooftree}

To have an impredicative universe, we take the rule
\begin{prooftree}\
    \AxiomC{$\Gamma , \lceil X \rceil : Val \vdash A \; small $}
    \UnaryInfC{$\Gamma \vdash \forall X . A \; small$}
\end{prooftree}

\blue{Q: By removing the type context and using universes, 
types (or codes in $Val$) are now impacted by bunched connectives? (see bunched polymorphism paper)
And what about having $\phi : \; Prop$ also in $\Gamma$? }

 

\section{TODO/Questions}
\begin{itemize}
    \item Do we add the type $\forall \underline{X}. \underline{B}$ to the object lang (\blue{yes}) and then encode $F A$ ?
    ...
    \item Check that the classification of logical connectives makes sense (value prop vs comp prop) (\blue{yes})
    \item \blue{yes}Does the body of a value relation need to be value propositions? (same question for computation relation)
    ...
    \item Attempt the relational interpretation of our new types \blue{cant internalize some of the interpretations, ex Case}
    ...
    \item PE Logic denotes the collection of computation relations, $Rel_{\mathcal{C}}[\underline{A},\underline{B}]$, by $Sub_{\mathcal{C}}(\underline{A} \times \underline{B})$. However,
          they never define $\underline{A} \times \underline{B}$ or state that it is a derivable type. \blue{we have cartesian product of computation types}
\end{itemize}
